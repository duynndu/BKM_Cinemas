                                ...
                                <div class="courses-bx">
                                    ...
                                    <div class="text-start d-flex align-items-center">
                                        @can('update', App\Models\Role::class)
                                            <a href="{{ route('admin.roles.edit', $role->id) }}" class="btn btn-primary btn-sm light">{{ __('language.admin.members.roles.editRole') }}</a>
                                        @endcan
                                        @can('delete', App\Models\Role::class)
                                            <form class="formDelete" action="{{ route('admin.roles.delete', $role->id) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" style="margin-left: 10px" class="btn btn-danger btn-sm light btnDelete">{{ __('language.admin.members.roles.deleteRole') }}</button>
                                            </form>
                                        @endcan
                                    </div>
                                </div>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="row mt-3">
                                        <table class="table table-bordered">
                                            <tr>
                                                <td style="width: 200px;">{{ __('language.admin.members.roles.name') }}</td>
                                                <td class="text-start">{{ $role->name ?? '' }}</td>
                                            </tr>
                                            <tr>
                                                <td style="width: 200px;">{{ __('language.admin.members.roles.description') }}</td>
                                    ...
                                    <div class="row mt-3">
                                        <div class="card">
                                            <div class="card-header border-0 pb-0 d-flex justify-content-center">
                                                <h4 class="card-title">{{ __('language.admin.members.roles.jurisdiction') }}</h4>
                                            </div>
                                            <div class="card-body p-0">
                                                <div id="DZ_W_TimeLine" class="widget-timeline dlab-scroll height370 my-4 px-4">
                                                    <ul class="timeline">
                                                        @if ($role->type !== App\Models\User::TYPE_ADMIN)
                                                            ...
                                                            @else
                                                                <li>
                                                                    ...
                                                                    <a class="timeline-panel text-muted">
                                                                        <h6 class="mb-0">{{ __('language.admin.members.roles.noPermission') }}</h6>
                                                                    </a>
                                                                </li>
                                                            @endif
                                                        @else
                                                            <li>
                                                                <div class="timeline-badge success"></div>
                                                                <a class="timeline-panel text-muted">
                                                                    <h6 class="mb-0">{{ __('language.admin.members.roles.fullPermission') }}</h6>
                                                                </a>
                                                            </li>
                                                        @endif
                                                    </ul>
